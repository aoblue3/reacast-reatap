...
/// (今回のように既存のmain.js/main.htmlを編集しただけのケース)では
/// ディレクトリ自体のmtimeは変化しないため、変更が無視されてしまっていた。
/// そのため中のファイル1つ1つを個別に列挙してcargoへ渡す必要がある。
...
fn main() {
  ...
  // されない」という形で現れ、原因の特定に何度も時間がかかった)。
  // frontendディレクトリ配下のファイルを1つずつ明示的にビルドスクリプトの
  // 再実行トリガーに加えることで、frontendだけを編集した場合でも確実に
  // 最新の内容が埋め込まれるようにする。
